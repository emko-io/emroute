# Setting Up markdown-it with emroute
...
CommonMark-compliant parser with a large plugin ecosystem and fine-grained
control over rendering rules.
...
Your `deno.json` imports should include both emroute and markdown-it:
...
When `html: true`, any HTML tag in the markdown source is included verbatim in
the output. This is useful for embedding custom elements or rich HTML in your
`.page.md` files, but is a security risk with untrusted content.
...
markdown-it has a larger footprint than marked (~362KB bundled vs ~129KB).
If bundle size is a concern and you don't need markdown-it's plugin ecosystem,
consider [marked](./setup-marked.md) instead.
