...
The HTML shell (generated by the server at request time) contains:
...
conflict) and writes the combined map to `importmap.json`. The server
inlines it into the HTML shell at request time.
...
loading. See the [widget documentation](widgets) for details.
