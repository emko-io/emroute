...
Deno.test(
  { name: "SPA mode 'only' — HTTP behavior", sanitizeResources: false, sanitizeOps: false },
  async (t) => {
  ...
  async (t) => {
    ...
    await t.step(
      ...
      async () => {
        ...
        await page.evaluate(async () =>
          await (globalThis as unknown as Record<string, { navigate(url: string): Promise<void> }>)
    ...
    await t.step(
      ...
      async () => {
        ...
        const params = await page.evaluate(
          () =>
            (globalThis as unknown as Record<string, { getParams(): Record<string, string> }>)
    ...
    await t.step(
      ...
      async () => {
        ...
        const widgetError = await page.waitForSelector('widget-failing .c-error', {
          timeout: 5000,
        });
